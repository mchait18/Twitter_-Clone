...
def messages_liked(user_id):
    """Show list of warblers that this user liked."""

    if g.user:
        liked_ids = [u.id for u in g.user.likes]
        ...
        messages = (Message
                    ...
                    .limit(100)
                    .all())
        likes_mids = [like.message_id for like in Likes.query.filter(Likes.user_id == g.user.id)]
        # breakpoint()
        return render_template('messages/liked.html', messages=messages, likes=likes_mids)
...
def homepage():
    ...
    if g.user:
        ...
        likes_mids = [like.message_id for like in Likes.query.filter(Likes.user_id == g.user.id)]
        # breakpoint()
        return render_template('home.html', messages=messages, likes=likes_mids)
